...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl shop to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.product-info .special-price')
        if _price_tags:
            try:
            ...
            _price_tags = driver.find_elements(By.CSS_SELECTOR, '.product-info .minimal-price .price')
            if _price_tags:
                try:
                    product_price = int(convert_to_english(_price_tags[0].text))
                except Exception:
        ...
        add_to_carts = driver.find_elements(By.CSS_SELECTOR, '.product-options-bottom .button.btn-cart')
        ...
        logging.error(f'Error in "crawl_price_name.customs.shop_ir": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the shop"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.product-img-box')
