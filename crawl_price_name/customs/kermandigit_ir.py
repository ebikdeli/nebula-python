"""kermandigit.ir needs js to be able to select color and read products"""
import settings
...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl kermandigit to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
            driver = call_selenium_driver(site_url=href, timeout=timeout, implicit_wait=settings.GENERAL_IMPLICIT_WAIT, headless=False, disable_css=False)
            if not driver:
                logging.error('Driver did not created')
                return None
        print('Kermandigit function')
        ...
        # Product_price
        color_list = driver.find_elements(By.CSS_SELECTOR, '.thwvsf_fields .thwvsf-wrapper-ul')
        if color_list:
            li_colors = color_list[0].find_elements(By.TAG_NAME, 'li')
            for lc in li_colors:
                if not 'deactive' in lc.get_attribute('class'):
                    lc.click()
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.cart-pro.price ins .woocommerce-Price-amount bdi')
        if _price_tags:
            try:
            ...
            _price_tags = driver.find_elements(By.CSS_SELECTOR, '.cart-pro.price .woocommerce-Price-amount bdi')
            if _price_tags:
                try:
                    product_price = int(convert_to_english(_price_tags[0].text))
                except Exception:
        ...
        add_to_carts = driver.find_elements(By.CSS_SELECTOR, '.product-seller-info .stock')
        for add_to_cart in add_to_carts:
            if 'ناموجود' in add_to_cart.text:
                is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.kermandigit_ir": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the kermandigit"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.woocommerce-product-gallery')
