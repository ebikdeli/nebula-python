"""We use requests instead of selenium for digieram.com"""
import settings
...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl digieram to get price, title, and image of a product"""
    try:
        print('Digieram function')
        ...
        add_to_carts = soup.select('.woocommerce-variation-add-to-cart')
        ...
        logging.error(f'Error in "crawl_price_name.customs.digieram_com": {e.__str__()}')
...
def find_image(soup:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the digieram"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
                ...
                return ''
        main_image_url = ''
        # _main_product_content = soup.select_one('.is-nav-selected')
        _main_product_content = soup.select_one('.woocommerce-product-gallery__wrapper.product-gallery-slider')
        if _main_product_content:
            image_content = _main_product_content.select('a')
            if image_content:
                main_image_url = image_content[0].get('href')
                # Save image
                if force_save_image:
