...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl tei to get price, title, and image of a product"""
    try:
        print('Tei function')
        ...
        # Product_price
        _price_tags = soup.select('.summary.entry-summary ins .woocommerce-Price-amount bdi')
        if _price_tags:
            try:
            ...
            _price_tags = soup.select('.summary.entry-summary .woocommerce-Price-amount bdi')
            if _price_tags:
                try:
                    product_price = int(convert_to_english(_price_tags[0].text))
                except Exception:
        ...
        logging.error(f'Error in "crawl_price_name.customs.tei_ir": {e.__str__()}')
