"""saving this website's image is rather complex"""
import settings
...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl hitoox to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        driver.get(href)
        # Product_title
        _title_tags = driver.find_elements(By.CSS_SELECTOR, '.product-name')
        if _title_tags:
            product_title = _title_tags[0].text
        # print('product title: ', product_title)
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.black-text.font-weight-bold strong')
        if _price_tags:
            try:
        ...
        add_to_carts = driver.find_elements(By.CSS_SELECTOR, '.btn-addtocard')
        for add_to_cart in add_to_carts:
            if 'خـریـد' in add_to_cart.text:
                is_available = True
        ...
        logging.error(f'Error in "crawl_price_name.customs.hitoox_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False, href:str=None) -> str|None:
    """Find product image in the hitoox"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.ngx-gallery-image.ng-trigger')
        if _main_product_content:
                main_image_url = _main_product_content[0].get_attribute('style')
                if main_image_url:
                    main_image_url = urlparse(href).scheme + '://' + urlparse(href).netloc + main_image_url[23:-3]
                # Save image
                if force_save_image:
