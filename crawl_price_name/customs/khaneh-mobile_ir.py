...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl khaneh-mobile to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        driver.get(href)
        # Product_title
        _title_tags = driver.find_elements(By.CSS_SELECTOR, '.product_title.entry-title')
        if _title_tags:
            product_title = _title_tags[0].text
        # print('product title: ', product_title)
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, 'meta[name="twitter:data1"]')
        if _price_tags:
            try:
                content = _price_tags[0].get_attribute('content')
                product_price = int(convert_to_english(content))
            except Exception:
                product_price = -1
        # print('product price: ', product_price)
        # If product is_available
        not_avail = driver.find_elements(By.CSS_SELECTOR, '.woocommerce-error')
        if not_avail:
            is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.khaneh-mobile_ir": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the khaneh-mobile"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.woocommerce-product-gallery.woocommerce-product-gallery--with-images.images product-images')
