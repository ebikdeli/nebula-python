...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl dramastore to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
            driver = call_selenium_driver(site_url=href, timeout=timeout, implicit_wait=settings.GENERAL_IMPLICIT_WAIT)
            if not driver:
                return None
        print('Dramastore function')
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.price-items .price-val')
        if _price_tags:
            try:
        ...
        logging.error(f'Error in "crawl_price_name.customs.dramastore_ir": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the dramastore"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.images-item')
