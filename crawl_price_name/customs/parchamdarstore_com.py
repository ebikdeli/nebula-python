...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl parchamdarstore to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
            ...
            if not driver:
                ...
                return None
        print('Parchamdarstore function')
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.summary-inner .woocommerce-variation-price .price ins .woocommerce-Price-amount bdi')
        if _price_tags:
            try:
            ...
            _price_tags = driver.find_elements(By.CSS_SELECTOR, '.summary-inner .woocommerce-variation-price .price .woocommerce-Price-amount bdi')
            if _price_tags:
                try:
                    # product_price = int(convert_to_english(_price_tags[0].text))
                    product_price = int(combine_parse_convert_tag(_price_tags[0]))
        ...
        logging.error(f'Error in "crawl_price_name.customs.parchamdarstore_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the parchamdarstore"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
