...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl market_irancell to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
            ...
            if not driver:
                ...
                return None
        print('Market_irancell function')
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.mantine-rtl-fix14e')
        if _price_tags:
            try:
        ...
        add_to_carts = driver.find_elements(By.CSS_SELECTOR, '.mantine-rtl-18novo4')
        ...
        logging.error(f'Error in "crawl_price_name.customs.market_irancell_ir": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the market_irancell"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
                ...
                return ''
        main_image_url = ''
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.mantine-rtl-Grid-col')
        # _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.mantine-rtl-1hm4z4b')
