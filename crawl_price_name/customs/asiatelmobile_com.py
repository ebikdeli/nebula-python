...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl asiatelmobile to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.product-info ins .woocommerce-Price-amount bdi')
        if _price_tags:
            try:
            ...
            _price_tags = driver.find_elements(By.CSS_SELECTOR, '.product-info .woocommerce-Price-amount bdi')
            if _price_tags:
                try:
                    product_price = int(convert_to_english(_price_tags[0].text))
                except Exception:
        ...
        outstock = driver.find_elements(By.CSS_SELECTOR, '.price-not-in-stock')
        if outstock:
            is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.asiatelmobile_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the asiatelmobile"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
