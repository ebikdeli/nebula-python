...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl gatamart to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        driver.get(href)
        # Product_title
        _title_tags = driver.find_elements(By.CSS_SELECTOR, '.title.page-title')
        if _title_tags:
            for _ in _title_tags:
                if _.text:
                    product_title = _.text
        # print('product title: ', product_title)
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.product-price-new')
        if _price_tags:
            try:
            ...
            _price_tags = driver.find_elements(By.CSS_SELECTOR, '.product-price')
            if _price_tags:
                try:
                    product_price = int(convert_to_english(_price_tags[0].text))
                except Exception:
        ...
        add_to_carts = driver.find_elements(By.CSS_SELECTOR, '.btn-cart')
        for add_to_cart in add_to_carts:
            if 'سبد خرید' in add_to_cart.text:
                is_available = True
        if product_price == 0:
            is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.gatamart_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the gatamart"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.swiper-slide.swiper-slide-visible.swiper-slide-active')
