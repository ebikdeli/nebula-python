"""dgdakke.com needs longer timeout"""
import settings
...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl dgdakke to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
            driver = call_selenium_driver(site_url=href, timeout=60, implicit_wait=settings.GENERAL_IMPLICIT_WAIT)
            if not driver:
                logging.error('Driver did not created')
                return None
        print('Dgdakke function')
        ...
        driver.get(href)
        # Product_title
        _title_tags = driver.find_elements(By.CSS_SELECTOR, '.product_title')
        if _title_tags:
            product_title = _title_tags[0].text
        # print('product title: ', product_title)
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.wd-single-price .price .woocommerce-Price-amount.amount bdi')
        if _price_tags:
            try:
        ...
        add_to_carts = driver.find_elements(By.CSS_SELECTOR, '.product-labels .out-of-stock ')
        for add_to_cart in add_to_carts:
            if 'اتمام موجودی' in add_to_cart.text:
                is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.dgdakke_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the dgdakke"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.product-image-wrap')
        if _main_product_content:
            _content_images = _main_product_content[0].find_elements(By.TAG_NAME, 'figure')
            if _content_images:
                main_image_url = _content_images[0].get_attribute('data-thumb')
                # Save image
                if force_save_image:
