...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl tellstar to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        if _price_tags:
            for _pt in _price_tags:
                try:
                    product_price = price_set.add(int(convert_to_english(_pt.text)))
                except Exception:
                    pass
        if price_set:
            try:
                product_price = min(price_set)
            except Exception:
                product_price = -1
        ...
        logging.error(f'Error in "crawl_price_name.customs.tellstar_ir": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the tellstar"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
