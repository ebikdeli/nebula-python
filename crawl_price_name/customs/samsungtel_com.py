...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl samsungtel to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.summary-inner p[class="price"] .woocommerce-Price-amount bdi')
        if _price_tags:
            for _pt in _price_tags:
                try:
                    product_price = _price_set.add(int(convert_to_english(_pt.text)))
                except Exception:
                    pass
        if 0 in _price_set:
            _price_set.remove(0)
        if _price_set:
            product_price = min(_price_set)
        if not _price_set:
            product_price = -1
        ...
        logging.error(f'Error in "crawl_price_name.customs.samsungtel_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the samsungtel"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
