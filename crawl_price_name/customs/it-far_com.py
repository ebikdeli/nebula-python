...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl it-far to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        outstocks = driver.find_elements(By.CSS_SELECTOR, '.single_add_to_cart_button.disabled')
        if outstocks:
            is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.it-far_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the it-far"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
                ...
                return ''
        main_image_url = ''
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.imgs-desctop')
