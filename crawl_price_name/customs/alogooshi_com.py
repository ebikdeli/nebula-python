"""alogooshi.com needs js to load color to load the price"""
import settings
...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl alogooshi to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        driver.get(href)
        # Product_title
        _title_tags = driver.find_elements(By.TAG_NAME, 'h2')
        if _title_tags:
            product_title = _title_tags[0].text
        # print('product title: ', product_title)
        # Product_price
        # Click on color box to show price
        _color_box = driver.find_elements(By.CSS_SELECTOR, '#ContentPlaceHolder1_Repeater5_gco_0 .val label')
        if _color_box:
            for cb in _color_box:
                if cb.tag_name == 'label':
                    cb.click()
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '#ContentPlaceHolder1_Repeater5_label6_0')
        if _price_tags:
            try:
        ...
        add_to_carts = driver.find_elements(By.CSS_SELECTOR, '.detail_body .future_item.col-xl-12 .val')
        for add_to_cart in add_to_carts:
            if 'موجود' in add_to_cart.text:
                is_available = True
        ...
        logging.error(f'Error in "crawl_price_name.customs.alogooshi_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the alogooshi"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.xzoom-container')
