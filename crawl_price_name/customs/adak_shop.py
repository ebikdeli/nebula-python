"""adak.shop redirected every time opened. So we should wait some seconds(5) till any page got loaded up"""
import settings
...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl adak to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
            driver = call_selenium_driver(site_url=href, timeout=timeout, implicit_wait=settings.GENERAL_IMPLICIT_WAIT, disable_css=False, headless=False)
            if not driver:
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.single_variation_wrap .woocommerce-Price-amount bdi')
        if _price_tags:
            try:
        ...
        add_to_carts = driver.find_elements(By.CSS_SELECTOR, '.stock.out-of-stock')
        for add_to_cart in add_to_carts:
            if 'در انبار موجود نیست' in add_to_cart.text:
                is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.adak_shop": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the adak"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.product-image .fancybox')
