...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl rojafon to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.product-option .price-text strong')
        if _price_tags:
            try:
        ...
        outstocks = driver.find_elements(By.CSS_SELECTOR, '.product-option .price-text')
        for outstock in outstocks:
            if 'موجود نیست' in outstock.text:
                is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.rojafon_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the rojafon"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.img-preview')
