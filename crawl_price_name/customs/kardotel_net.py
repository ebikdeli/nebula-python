...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl kardotel to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, 'h6 span')
        for _pt in _price_tags:
            if _pt.text:
                product_prices.add(int(convert_to_english(_pt.text)))
        if 0 in product_prices or -1 in product_prices:
            product_prices.remove(0)
        if product_prices:
        ...
        add_to_cart_btns = driver.find_elements(By.TAG_NAME, 'button')
        for btn in add_to_cart_btns:
            if 'افزودن به سبد' in btn.text:
                is_available = True
        ...
        logging.error(f'Error in "crawl_price_name.customs.kardotel_net": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the kardotel"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.gallery-box')
