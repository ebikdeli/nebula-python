...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl fopit to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        driver.get(href)
        # Product_title
        _h2_tags = driver.find_elements(By.CSS_SELECTOR, '.kala_text .titr_kala h2')
        if _h2_tags:
            product_title = _h2_tags[0].text
        # print('product title: ', product_title)
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.gheimat_kala .prce_wundeful_op')
        if _price_tags:
            try:
        ...
        add_to_carts = driver.find_elements(By.CSS_SELECTOR, '.mojodr')
        ...
        logging.error(f'Error in "crawl_price_name.customs.fopit_ir": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the fopit"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.aks_kala')
