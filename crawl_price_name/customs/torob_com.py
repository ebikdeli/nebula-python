...
def price_title_image(href:str, site_id:bool, link_id:bool, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl torob to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
            driver = call_selenium_driver(site_url=href, timeout=timeout, implicit_wait=settings.TOROB_IMPLICIT_WAIT)
            if not driver:
                logging.error('Driver did not created')
                return None
        print('torob function')
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.jsx-63b317fab2efbae.buy_box_text.ellipsis')
        if _price_tags:
            try:
                product_price = int(_price_tags[1].text.split(' ')[0].replace(chr(1643), ''))
            except Exception:
                product_price = -1
        ...
        logging.error(f'Error in "crawl_price_name.customs.torob_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the torob"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        if _main_product_content:
            ...
            if _content_images:
                ...
                if force_save_image:
                    ...
                    _new_image = _original_image.resize((600, 480))
