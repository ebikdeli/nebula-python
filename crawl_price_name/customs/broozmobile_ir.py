...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl broozmobile to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.elementor-element-55b4b755 .price .woocommerce-Price-amount.amount bdi')
        if _price_tags:
            prices = set()
            try:
                # for _pt in _price_tags:
                #     print(_pt.text)
                #     prices.add(int(combine_parse_convert_tag(_pt)))
                product_price = int(combine_parse_convert_tag(_price_tags[-1]))
        ...
        in_stock = driver.find_elements(By.CSS_SELECTOR, '.stock')
        for _ in in_stock:
            if 'موجود' in _.text:
                is_available = True
        ...
        logging.error(f'Error in "crawl_price_name.customs.broozmobile_ir": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the broozmobile"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
