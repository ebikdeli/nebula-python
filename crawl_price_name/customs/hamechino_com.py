"""We must use requests library instead of selenium for hemechino.com"""
import settings
...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl hamechino to get price, title, and image of a product"""
    try:
        print('Hamechino function')
        ...
        main_image = ''
        # Product_title
        _h1_tags = soup.select_one('.Pname')
        if _h1_tags:
            product_title = _h1_tags.text
        # print('product title: ', product_title)
        # Product_price
        _price_tags = soup.select('.single_box_price .rem1-2')
        if _price_tags:
            try:
        ...
        # add_to_carts = soup.select('.elementor-element.elementor-element-3eb9ff3.elementor-widget.elementor-widget-AddToCart')
        add_to_carts = soup.select('.add_tab_stock')
        for add_to_cart in add_to_carts:
            if 'موجود در انبار' in add_to_cart.text and product_price > 0:
                is_available = True
        ...
        logging.error(f'Error in "crawl_price_name.customs.hamechino_com": {e.__str__()}')
...
def find_image(soup:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the hamechino"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = soup.select('.elementor-widget-vitrin_product_image')
        if _main_product_content:
            _content_images = _main_product_content[0].select_one('img')
            if _content_images:
                main_image_url = _content_images.get('src')
                # Save image
                if force_save_image:
