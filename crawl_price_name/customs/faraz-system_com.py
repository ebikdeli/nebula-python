...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl faraz-system to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        driver.get(href)
        # Product_title
        title_tags = driver.find_elements(By.CSS_SELECTOR, '.product_title')
        if title_tags:
            product_title = title_tags[0].text
        # print('product title: ', product_title)
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.elementor-location-single ins .woocommerce-Price-amount bdi')
        if _price_tags:
            try:
                ...
                product_price = -1
        if not _price_tags or int(product_price) == -1 or int(product_price) == 0:
            _price_tags = driver.find_elements(By.CSS_SELECTOR, '.elementor-location-single .woocommerce-Price-amount bdi')
            if _price_tags:
                try:
                    # product_price = int(convert_to_english(_price_tags[0].text))
                    product_price = int(combine_parse_convert_tag(_price_tags[0]))
                    ...
                    product_price = -1
        if product_price > 0:
            product_price = int(product_price / 10)
        ...
        logging.error(f'Error in "crawl_price_name.customs.faraz-system_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the faraz-system"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
                ...
                return ''
        main_image_url = ''
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.elementor-location-single .elementor-hidden-mobile.elementor-widget.elementor-widget-image')
        if _main_product_content:
            ...
            if _content_images:
                ...
                # main_image_url = _content_images[0].get_attribute('srcset')
                # Save image
                if force_save_image:
