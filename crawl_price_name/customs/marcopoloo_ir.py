...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl marcopoloo to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '.elementor-widget-wd_single_product_price ins .woocommerce-Price-amount bdi')
        if _price_tags:
            try:
            ...
            _price_tags = driver.find_elements(By.CSS_SELECTOR, '.elementor-widget-wd_single_product_price .woocommerce-Price-amount bdi')
            if _price_tags:
                if len(_price_tags) < 2:
                    try:
                        product_price = int(convert_to_english(_price_tags[0].text))
                    except Exception:
                        product_price = -1
                    ...
                    price_set = set()
                    for _pt in _price_tags:
                        try:
                            price_set.add(int(convert_to_english(_pt.text)))
                        except Exception:
                            continue
                    if 0 in price_set:
                        price_set.remove(0)
                    product_price = min(price_set) if price_set else -1
        # If product is_available
        outstocks = driver.find_elements(By.CSS_SELECTOR, '.stock.out-of-stock')
        if outstocks:
            is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.marcopoloo_ir": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the marcopoloo"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, '.woocommerce-product-gallery__image')
