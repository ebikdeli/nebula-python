...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl masterkala to get price, title, and image of a product. If selenium driver could not get launched return None"""
    try:
        if not driver:
        ...
        # Product_price
        _price_tags = driver.find_elements(By.CSS_SELECTOR, '#product-row .primary--text.text-h6')
        if _price_tags:
            try:
        ...
        outstocks = driver.find_elements(By.CSS_SELECTOR, '.unavailable-product')
        if outstocks:
            is_available = False
        ...
        logging.error(f'Error in "crawl_price_name.customs.masterkala_com": {e.__str__()}')
...
def find_image(driver:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    """Find product image in the masterkala"""
    try:
        if force_save_image:
            try:
                site_id = str(site_id)
        ...
        _main_product_content = driver.find_elements(By.CSS_SELECTOR, 'meta[name="og:image"]')
        if _main_product_content:
            _content_images = _main_product_content[0].get_attribute('content')
            if _content_images:
                _content_images = _content_images.replace(' ', '%20')
                main_image_url = _content_images
                # Save image
                if force_save_image:
