...
def price_title_image(href:str, site_id:int=None, link_id:int=None, save_image:bool=False, force_save_image:bool=False, driver:object=None, timeout:int=settings.CHROME_DRIVER_TIMEOUT) -> list|None:
    """Crawl SITENAME to get price, title, and image of a product"""
    try:
        print('SITENAME function')
        ...
        if _h1_tags:
            product_title = _h1_tags.text.strip()
        # print('product title: ', product_title)
        # Product_price
        _price_tags = soup.select('.product-info-main .color-aymbol')
        if _price_tags:
            try:
        ...
        add_to_carts = soup.select('#product-addtocart-button')
        if add_to_carts:
            is_available = True
...
def find_image(soup:object, site_id:int, link_id:int, force_save_image:bool=False) -> str|None:
    ...
    try:
        ...
        _main_product_content = soup.select('.product-left')
